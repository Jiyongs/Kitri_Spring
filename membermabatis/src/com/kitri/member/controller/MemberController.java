...
//		session.removeAttribute("userInfo");
	...
	public String getMember(HttpServletRequest request, HttpServletResponse response) {
		...
		// 세션으로부터 로그인 한 계정의 id 값 받기
		HttpSession session = request.getSession();
		MemberDto memberDto = (MemberDto) session.getAttribute("userInfo");

		String id = memberDto.getId();
		
		MemberDetailDto memberInfo = MemberServiceImpl.getMemberService().getMember(id);
		
		request.setAttribute("memberInfo", memberInfo);
		
		return "/user/member/modify.jsp";
	}
	
	public String modifyMember(HttpServletRequest request, HttpServletResponse response) {
		
		// TODO 정보수정 (modify.jsp + 기능 구현 해야 함!)
		// 1 수정한 정보 받기 (getParameter 이용)
		MemberDetailDto memberInfo = null;
		
		int result = MemberServiceImpl.getMemberService().modifyMember(memberInfo);
		
		// 2 수정 성공 시,
		if(result != 0) {
			// session에 수정된 정보 세팅하기
		}
		
		return "/user/login/loginok.jsp";
	}
